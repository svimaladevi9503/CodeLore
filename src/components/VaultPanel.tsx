...
import { Database, FolderOpen, History, LayoutGrid, ServerCrash, Check, FileCode } from "lucide-react";
      ...
      <div className="flex flex-col gap-6 h-full font-sans">
        <div className="flex items-center justify-between border-b border-slate-900 pb-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[14px] font-sans font-medium text-slate-200">CodeLore Vault</h3>
          </div>
  ...
  return (
    <div className="flex flex-col gap-6 h-full font-sans">
      ...
      <div className="flex-1 overflow-y-auto space-y-6 select-none pr-1">
        <ReadmeHistorySection
          readmes={parcleData.readmes}
        ...
        {isCleanerActive ? (
          <CleanerPatchLogSection parcleData={parcleData} repoName={repoName} />
        ) : (
          <KnowledgeIndexSection parcleData={parcleData} setActiveTab={setActiveTab} />
        )}

        <div className="h-px bg-slate-900 border-none" />

        <PatchLogSection parcleData={parcleData} repoName={repoName} setActiveTab={setActiveTab} />
      </div>
    </div>
  );
}

function ReadmeHistorySection({ readmes, setHistoricDiffOverlay }: { readmes: any; setHistoricDiffOverlay: any }) {
  const readmeEntries = Object.entries(readmes);
  return (
          ...
          {readmeEntries.map(([key, val]: any) => (
            ...
            >
              ...
              <div className="text-right shrink-0">
                <span className="text-blue-400 block font-medium">#{val.sha ? val.sha.substring(0, 5) : key.substring(0, 5)}</span>
...
function CleanerPatchLogSection({ parcleData, repoName }: { parcleData: ParcleRecord; repoName: string }) {
  const patchLog = useMemo(() => {
    ...
    <div className="flex flex-col gap-3">
        ...
        <div className="bg-slate-950/20 border border-slate-900 p-2.5 rounded-lg flex flex-col gap-1.5">
          <div className="flex items-center justify-between text-[10px] font-mono">
            ...
            <span className="text-slate-400">
              {lastScan.timestamp ? new Date(lastScan.timestamp).toLocaleDateString() : "—"}
            </span>
          </div>
          <div className="flex items-center justify-between text-[10px] font-mono">
            <span className="text-slate-500">Issues found:</span>
            <span className="text-amber-400 font-medium">{lastScan.total_issues || 0}</span>
          </div>
          <div className="flex items-center justify-between text-[10px] font-mono">
            <span className="text-slate-500">Files scanned:</span>
            <span className="text-slate-300">{lastScan.files_scanned || 0}</span>
          </div>
        </div>
      )}
      ...
      {patchLog.length === 0 ? (
        ...
        <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
          {patchLog.slice(-8).reverse().map((patch: any) => (
            <div
              key={`patch-${patch.id || patch.timestamp}`}
...
function KnowledgeIndexSection({ parcleData, setActiveTab }: { parcleData: ParcleRecord; setActiveTab: any }) {
      ...
      for (const [key, val] of Object.entries(parcleData.metadata)) {
        if (key.startsWith("kb:") && key !== "kb:index:manifest" && !key.endsWith(":prev")) {
          ...
          if (chunk && chunk.filename) {
            list.push({ filename: chunk.filename, section: chunk.header || "Section" });
          }
        }
      }
    }
...
function PatchLogSection({ parcleData, repoName, setActiveTab }: { parcleData: ParcleRecord; repoName: string; setActiveTab: any }) {
  const patchLog = useMemo(() => {
    const key = `cleaner:patch_log:${repoName}`;
    const log = parcleData.metadata?.[key];
    return Array.isArray(log) ? log : [];
    ...
    <div className="flex flex-col gap-3">
      ...
      {patchLog.length === 0 ? (
        ...
        <div className="flex flex-col gap-1.5 max-h-[150px] overflow-y-auto">
          {patchLog.slice(-3).reverse().map((patch: any) => (
            <button
              key={`patch-history-${patch.id || patch.timestamp}`}
              type="button"
