...
export default function GitHubIntegrationCard(props: GitHubIntegrationCardProps) {
  if (!props.ghUser) {
    return <UnauthGitHubIntegration {...props} />;
  }
  return <AuthGitHubIntegration {...props} />;
}

function UnauthGitHubIntegration({
  theme,
  inputToken,
  setInputToken,
  loadingGh,
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h4 className={`text-[14px] font-sans font-medium flex items-center gap-2 ${theme === "dark" ? "text-slate-300" : "text-slate-850"}`}>
          <Github className="h-4 w-4 text-purple-400" />
          <span>GitHub repository integration</span>
        </h4>
        <span className={`text-[11px] font-mono ${theme === "dark" ? "text-slate-500" : "text-slate-400"}`}>
          Authentication required
        </span>
      </div>

      <div className="flex flex-col gap-3">
        <p className={`text-[12px] ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>
          ...
          >
            <input
              ...
              className={`flex-1 border rounded-lg text-[12px] font-sans font-normal p-2.5 outline-none focus:border-purple-500 transition ${
                theme === "dark" ? "bg-slate-900 border-slate-800 text-slate-200 placeholder-slate-600" : "bg-white border-slate-200 text-slate-800 placeholder-slate-400"
              }`}
            />
            <button
              type="submit"
              ...
              className={`px-4 py-2.5 rounded-lg font-sans text-[12px] font-medium cursor-pointer flex items-center justify-center gap-1.5 active:scale-95 transition disabled:opacity-50 shadow-sm border ${
                theme === "dark" ? "bg-slate-900 border-teal-500/80 hover:bg-slate-850 text-teal-300" : "bg-white border-teal-500/80 hover:bg-teal-50/50 text-teal-700"
              }`}
            >
              {loadingGh ? (
        ...
        {errorGh && <p className="text-[11px] text-red-400 mt-1 font-sans" role="alert">{errorGh}</p>}
      </div>
    </div>
  );
}

function AuthGitHubIntegration({
  theme,
  ghUser,
  ghRepos,
  repoName,
  ...
  const filteredRepos = useMemo(() => {
    ...
    return ghRepos.filter((r) => r.full_name.toLowerCase().includes(term));
  }, [ghRepos, repoSearch]);
    ...
    >
      ...
      <div
        className={`flex flex-col md:flex-row items-center md:justify-between gap-4 bg-transparent border p-4.5 rounded-xl transition-all duration-250 ${
          theme === "dark" ? "border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.05)]" : "border-purple-500/40 shadow-[0_0_15px_rgba(168,85,247,0.03)]"
        }`}
      >
        <div className="flex items-center gap-3 w-full md:w-auto">
          <img
            ...
            alt={ghUser.login}
            className={`w-9 h-9 rounded-full border ${theme === "dark" ? "border-purple-500/30" : "border-purple-500/40"}`}
          />
          <div className="flex flex-col">
            <span className={`text-[12px] font-sans font-semibold ${theme === "dark" ? "text-slate-200" : "text-slate-800"}`}>
              @{ghUser.login}
            </span>
            <span className={`text-[10px] font-mono ${theme === "dark" ? "text-slate-500" : "text-slate-450"}`}>
        ...
        <div className="flex flex-col gap-1 w-full md:w-[260px] relative">
          <label htmlFor="repo-select" className={`text-[10px] font-sans font-medium ${theme === "dark" ? "text-slate-400" : "text-slate-500"}`}>
            Target repository
          </label>
          ...
          <button
            ...
            className={`flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg border text-[12px] font-sans font-normal cursor-pointer transition-all outline-none w-full ${
              repoDropdownOpen
                ? theme === "dark" ? "border-purple-500 bg-slate-900" : "border-purple-500 bg-white"
                : theme === "dark" ? "border-slate-800 bg-slate-950 hover:border-slate-700" : "border-slate-200 bg-white hover:border-slate-300"
            }`}
          >
            <span className={`truncate ${repoName ? (theme === "dark" ? "text-slate-100" : "text-slate-800") : (theme === "dark" ? "text-slate-500" : "text-slate-400")}`}>
              {repoName ? ghRepos.find((r) => r.name === repoName)?.full_name ?? repoName : "Select repository..."}
            </span>
            <ChevronDown className={`h-3.5 w-3.5 shrink-0 transition-transform duration-200 ${repoDropdownOpen ? "rotate-180 text-purple-400" : (theme === "dark" ? "text-slate-500" : "text-slate-400")}`} />
          </button>
            ...
            {repoDropdownOpen && (
              <>
                ...
                <m.div
                  ...
                  style={{ maxHeight: 260 }}
                >
                  <div className={`flex items-center gap-2 px-3 py-2 border-b ${theme === "dark" ? "border-slate-800" : "border-slate-100"}`}>
                    <Search className={`h-3.5 w-3.5 shrink-0 ${theme === "dark" ? "text-slate-500" : "text-slate-400"}`} />
                    <input
                      type="text"
                  ...
                  <div className="overflow-y-auto" style={{ maxHeight: 200 }}>
                    {filteredRepos.length === 0 ? (
                      <div className={`px-4 py-3 text-[12px] text-center ${theme === "dark" ? "text-slate-500" : "text-slate-400"}`}>
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left flex items-center justify-between gap-2 px-3 py-2.5 text-[12px] font-sans transition-colors cursor-pointer ${
                              isSelected
                                ? theme === "dark" ? "bg-purple-500/10 text-purple-300" : "bg-purple-550/10 text-purple-700"
                                : theme === "dark" ? "text-slate-300 hover:bg-slate-850" : "text-slate-700 hover:bg-slate-50"
                            }`}
                          >
      ...
      <div className={`text-[11px] font-mono pl-1 flex items-center gap-1.5 ${theme === "dark" ? "text-slate-500" : "text-slate-450"}`}>
        <FolderGit className="h-3.5 w-3.5 text-teal-400" />
        <span>
          Active context: <strong className={theme === "dark" ? "text-slate-300" : "text-slate-700"}>{repoName || "None selected"}</strong>
        </span>
      </div>
    </div>
  );
